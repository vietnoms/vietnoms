...
export function CheckoutPanel() {
  const { items, total, clearCart, updateQuantity, removeItem, closeCheckout } = useCart();
  ...
  return (
    <div ref={panelRef} className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="bg-surface-alt rounded-lg overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-brand-red">{formatPrice(orderTotals?.subtotal ?? total)}</span>
              <ChevronDown className={`h-4 w-4 text-gray-400 transition-transform ${orderExpanded ? "rotate-180" : ""}`} />
            </div>
          </button>
          {orderExpanded && (
              ...
              {items.map((item, i) => (
                <div key={i} className="flex gap-2 text-sm">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <button
                        ...
                        className="h-5 w-5 rounded border border-gray-600 flex items-center justify-center text-gray-400 hover:text-white hover:border-gray-400 transition-colors"
                      >
                        <Minus className="h-2.5 w-2.5" />
                      </button>
                      <span className="text-gray-300 text-xs w-4 text-center">{item.quantity}</span>
                      <button
                        onClick={() => updateQuantity(i, item.quantity + 1)}
                        className="h-5 w-5 rounded border border-gray-600 flex items-center justify-center text-gray-400 hover:text-white hover:border-gray-400 transition-colors"
                      >
                  ...
                  <div className="flex flex-col items-end justify-between">
                    <span className="text-white text-xs">{formatPrice(item.lineTotal)}</span>
                    <button
        ...
        {/* Totals */}
        <div className="bg-surface-alt rounded-lg p-3 space-y-1.5 text-sm">
        ...
        {/* Hours banner */}
        {hoursDisplay && (
          <div className="flex items-center gap-2 text-xs px-3 py-2 bg-surface-alt rounded-lg">
            <Clock className="h-3.5 w-3.5 text-gray-400 shrink-0" />
            <span className="text-gray-400">Open today: {hoursDisplay}</span>
            {!restaurantOpen && (
              <span className="text-red-500 font-medium ml-auto text-xs">Closed</span>
            )}
            {restaurantOpen && !acceptingOrders && (
              <span className="text-amber-600 font-medium ml-auto text-xs">Closing soon</span>
            )}
          </div>
        )}

        {closedMessage && (
          <div className="p-3 bg-amber-900/30 border border-amber-800 rounded-lg text-xs">
        ...
        <section className="space-y-3">
          ...
          <div>
            ...
            <div className="relative">
              <Input id="cp-phone" type="tel" required value={customerInfo.phone}
        ...
        {/* PICKUP */}
        <section className="space-y-3">
          <h3 className="font-display text-base font-bold text-white">Pickup</h3>
          <div className="flex gap-2">
            <button type="button" className={`flex-1 py-2 px-3 rounded-lg border text-sm font-medium transition-colors ${pickupMode === "asap" ? "border-brand-red bg-brand-red/5 text-brand-red" : "border-gray-600 text-gray-300 hover:border-gray-300"}`}
              onClick={() => {
                if (!restaurantOpen) {
                  const nextOpen = getNextOpeningTime(new Date());
                  if (nextOpen) {
            ...
            <button type="button" className={`flex-1 py-2 px-3 rounded-lg border text-sm font-medium transition-colors ${pickupMode === "scheduled" ? "border-brand-red bg-brand-red/5 text-brand-red" : "border-gray-600 text-gray-300 hover:border-gray-300"}`}
              onClick={() => setPickupMode("scheduled")}>Schedule</button>
          </div>
          ...
          <div>
            <Label htmlFor="cp-notes">Special Instructions</Label>
            <Textarea id="cp-notes" value={customerInfo.notes}
        ...
        {/* TIP */}
        <section className="space-y-3">
          <h3 className="font-display text-base font-bold text-white">Tip</h3>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => { setTipAmount(0); setTipPercent(null); setTipMode("preset"); setCustomTip(""); }}
              className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${tipMode === "preset" && tipPercent === null && tipAmount === 0 ? "border-brand-red bg-brand-red/5 text-brand-red" : "border-gray-600 text-gray-300 hover:border-gray-300"}`}>
              No tip
            </button>
            {TIP_PERCENT_PRESETS.map((pct) => {
              ...
              return (
                <button key={pct} type="button"
                  ...
                  className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${tipMode === "preset" && tipPercent === pct ? "border-brand-red bg-brand-red/5 text-brand-red" : "border-gray-600 text-gray-300 hover:border-gray-300"}`}>
                  <span>{pct}%</span>
            ...
            <button type="button" onClick={() => { setTipMode("custom"); setTipPercent(null); setTipAmount(0); }}
              className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${tipMode === "custom" ? "border-brand-red bg-brand-red/5 text-brand-red" : "border-gray-600 text-gray-300 hover:border-gray-300"}`}>
              Custom
            </button>
          </div>
          {tipMode === "custom" && (
        ...
        {loyalty?.account && loyalty.program && (
          <section className="bg-yellow-900/20 border border-yellow-800/50 rounded-lg p-3 space-y-2">
        ...
        <section className="space-y-3">
          ...
          <label className="flex items-start gap-2 cursor-pointer">
            ...
            <span className="text-xs text-gray-400">Opt-in to receive promos and special deals via e-mail</span>
          </label>
        </section>

        {/* PAYMENT */}
        <section className="space-y-3">
          <h3 className="font-display text-base font-bold text-white">Payment</h3>
          {submitting && (
